...
@Component({
  template: `
    <table>
      <thead>
        <tr yaliSort [(predicate)]="predicate" [(ascending)]="ascending" (sortChange)="transition($event)">
          <th yaliSortBy="name">ID<fa-icon *ngIf="sortAllowed" [icon]="'sort'"></fa-icon></th>
        </tr>
      </thead>
    </table>
  `,
...
describe('Directive: SortByDirective', () => {
  ...
  it('should initialize predicate, order, icon when initial component predicate differs from column predicate', () => {
    ...
    // THEN
    expect(sortByDirective.yaliSortBy).toEqual('name');
