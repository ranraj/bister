...
type RestOf<T extends IProduct | NewProduct> = Omit<T, 'dateCreated' | 'dateModified'> & {
...
export class ProductService {
  ...
  protected convertDateFromClient<T extends IProduct | NewProduct | PartialUpdateProduct>(product: T): RestOf<T> {
    return {
      ...product,
      dateCreated: product.dateCreated?.toJSON() ?? null,
  ...
  protected convertDateFromServer(restProduct: RestProduct): IProduct {
    return {
      ...restProduct,
      dateCreated: restProduct.dateCreated ? dayjs(restProduct.dateCreated) : undefined,
