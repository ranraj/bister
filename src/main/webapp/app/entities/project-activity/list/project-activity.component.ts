...
import { EntityArrayResponseType, ProjectActivityService } from '../service/project-activity.service';
import { ProjectActivityDeleteDialogComponent } from '../delete/project-activity-delete-dialog.component';

@Component({
  selector: 'yali-project-activity',
...
export class ProjectActivityComponent implements OnInit {
  ...
  trackId = (_index: number, item: IProjectActivity): number => this.projectActivityService.getProjectActivityIdentifier(item);

  search(query: string): void {
    if (query && ProjectActivityComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (ProjectActivityComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.projectActivities = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IProjectActivity[] | null): IProjectActivity[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.projectActivityService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.projectActivityService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
