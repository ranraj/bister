...
import { EntityArrayResponseType, ProjectTypeService } from '../service/project-type.service';
import { ProjectTypeDeleteDialogComponent } from '../delete/project-type-delete-dialog.component';

@Component({
  selector: 'yali-project-type',
...
export class ProjectTypeComponent implements OnInit {
  ...
  trackId = (_index: number, item: IProjectType): number => this.projectTypeService.getProjectTypeIdentifier(item);

  search(query: string): void {
    if (query && ProjectTypeComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (ProjectTypeComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.projectTypes = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IProjectType[] | null): IProjectType[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.projectTypeService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.projectTypeService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
