...
type RestOf<T extends IProject | NewProject> = Omit<T, 'dateCreated' | 'dateModified'> & {
...
export class ProjectService {
  ...
  search(req: SearchWithPagination): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
      .get<RestProject[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
      .pipe(map(res => this.convertResponseArrayFromServer(res)));
  }
  ...
  protected convertDateFromClient<T extends IProject | NewProject | PartialUpdateProject>(project: T): RestOf<T> {
    return {
      ...project,
      dateCreated: project.dateCreated?.toJSON() ?? null,
      dateModified: project.dateModified?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProject: RestProject): IProject {
    return {
      ...restProject,
      dateCreated: restProject.dateCreated ? dayjs(restProject.dateCreated) : undefined,
      dateModified: restProject.dateModified ? dayjs(restProject.dateModified) : undefined,
    };
  }
