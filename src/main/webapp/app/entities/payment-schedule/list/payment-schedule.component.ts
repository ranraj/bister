...
import { EntityArrayResponseType, PaymentScheduleService } from '../service/payment-schedule.service';
import { PaymentScheduleDeleteDialogComponent } from '../delete/payment-schedule-delete-dialog.component';

@Component({
  selector: 'yali-payment-schedule',
  templateUrl: './payment-schedule.component.html',
})
export class PaymentScheduleComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['remarks', 'status'];
  ...
  trackId = (_index: number, item: IPaymentSchedule): number => this.paymentScheduleService.getPaymentScheduleIdentifier(item);

  search(query: string): void {
    if (query && PaymentScheduleComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (PaymentScheduleComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.paymentSchedules = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IPaymentSchedule[] | null): IPaymentSchedule[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.paymentScheduleService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.paymentScheduleService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
