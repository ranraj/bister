...
import { EntityArrayResponseType, TaxClassService } from '../service/tax-class.service';
import { TaxClassDeleteDialogComponent } from '../delete/tax-class-delete-dialog.component';

@Component({
  selector: 'yali-tax-class',
...
export class TaxClassComponent implements OnInit {
  ...
  trackId = (_index: number, item: ITaxClass): number => this.taxClassService.getTaxClassIdentifier(item);

  search(query: string): void {
    if (query && TaxClassComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (TaxClassComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.taxClasses = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ITaxClass[] | null): ITaxClass[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.taxClassService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.taxClassService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
