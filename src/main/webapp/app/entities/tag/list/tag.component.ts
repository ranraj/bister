...
export class TagComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['name', 'slug', 'description', 'tagType'];
  ...
  search(query: string): void {
    if (query && TagComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (TagComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
