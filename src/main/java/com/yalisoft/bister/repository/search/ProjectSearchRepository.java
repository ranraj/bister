 ...
 */
public interface ProjectSearchRepository extends ReactiveElasticsearchRepository<Project, Long>, ProjectSearchRepositoryInternal {}

interface ProjectSearchRepositoryInternal {
    Flux<Project> search(String query, Pageable pageable);

    Flux<Project> search(Query query);
}
...
class ProjectSearchRepositoryInternalImpl implements ProjectSearchRepositoryInternal {
    ...
    @Override
    public Flux<Project> search(String query, Pageable pageable) {
    ...
    @Override
    public Flux<Project> search(Query query) {
        return reactiveElasticsearchTemplate.search(query, Project.class).map(SearchHit::getContent);
    }
}
