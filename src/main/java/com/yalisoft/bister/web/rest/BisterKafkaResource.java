...
import com.yalisoft.bister.config.KafkaSseConsumer;
import com.yalisoft.bister.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/bister-kafka")
public class BisterKafkaResource {

    private final Logger log = LoggerFactory.getLogger(BisterKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public BisterKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
