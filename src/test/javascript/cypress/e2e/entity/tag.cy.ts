...
describe('Tag e2e test', () => {
  ...
  const tagSample = { name: 'Grass-roots base Nevada', slug: 'navigate', description: 'Knolls primary Markets', tagType: 'Product' };
  ...
  describe('new Tag page', () => {
    ...
    it('should create an instance of Tag', () => {
      ...
      cy.get(`[data-cy="slug"]`).type('Wooden').should('have.value', 'Wooden');

      cy.get(`[data-cy="description"]`).type('exploit Security Product').should('have.value', 'exploit Security Product');

      cy.get(`[data-cy="tagType"]`).select('Product');
