...
describe('Customer e2e test', () => {
  ...
  // const customerSample = {"name":"Kids cross-platform","contactNumber":"bluetooth clien"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/addresses',
      body: {"name":"HDDXXXXXXXXXXXXXXXXX","addressLine1":"Internal","addressLine2":"Granite Dollar Assurance","landmark":"Mayen","city":"Woodbury","state":"Licensed interactive","country":"Sudan","postcode":"innovate Pizza Visio","latitude":"Intelligent Configurable Savings","longitude":"1080p Frozen","addressType":"TEMPORARY_RESIDENT"},
    }).then(({ body }) => {
      address = body;
    });
  });
  ...
  describe('new Customer page', () => {
    ...
    it.skip('should create an instance of Customer', () => {
      ...
      cy.get(`[data-cy="contactNumber"]`).type('tealXXXXXX').should('have.value', 'tealXXXXXX');

      cy.get(`[data-cy="avatarUrl"]`).type('Swiss azure').should('have.value', 'Swiss azure');
